#!/bin/bash
# REDLINE Web GUI Multi-Platform Docker Deployment Script
...
print_header() {
    echo -e "${PURPLE}================================${NC}"
    echo -e "${PURPLE}  REDLINE Multi-Platform Deploy${NC}"
    echo -e "${PURPLE}================================${NC}"
}
...
show_usage() {
    ...
    echo ""
    echo "Examples:"
    echo "  $0                      # Full multi-platform deployment"
    echo "  $0 -p linux/amd64       # Deploy for AMD64 only"
    echo "  $0 -b --push            # Build and push to registry"
    echo "  $0 --no-multiarch       # Single platform deployment"
}
...
build_images() {
    ...
    # Use multi-platform build script if available
    if [[ -f "scripts/build-multiplatform.sh" ]]; then
        print_status "Using multi-platform build script"
        ...
        ./scripts/build-multiplatform.sh "${build_args[@]}"
    else
...
show_deployment_info() {
    ...
    echo "  • Multi-arch: $([ "$BUILDX_ENABLED" == true ] && echo "Enabled" || echo "Disabled")"
...
main() {
    ...
    print_success "Multi-platform deployment process completed!"
}
