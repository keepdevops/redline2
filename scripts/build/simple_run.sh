...
# REDLINE Web GUI - Simple Run Script
# Runs the BuildKit-free container
...
check_image() {
    if ! docker image inspect "$IMAGE_NAME:$TAG" >/dev/null 2>&1; then
        ...
        log "Please build the image first with: ./scripts/build/buildkit_free_build.sh"
        exit 1
    fi
...
run_container() {
    ...
    if [ "$MODE" = "detached" ]; then
        ...
        docker run -d \
            ...
            --volume "$PWD/logs:/opt/redline/logs" \
...
# Main function
main() {
    log "REDLINE Web GUI - Simple Runner"
    log "=============================="
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  - Image must be built with: ./scripts/build/buildkit_free_build.sh"
            exit 0
            ;;
        *)
