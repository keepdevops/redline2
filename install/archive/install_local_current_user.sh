...
# REDLINE Local Installation Script (Current User, No GitHub)
# Installs and configures REDLINE from local directory for current user

echo "🚀 REDLINE Local Installation Script (Current User)"
echo "=================================================="
...
# Configuration variables - use current user and local directory
CURRENT_USER="$USER"
REDLINE_HOME="$HOME"
REDLINE_DIR="$REDLINE_HOME/redline"
SOURCE_DIR="$(pwd)"  # Current directory where REDLINE is located
PYTHON_VERSION="3.11"
...
# Function to check if we're in a REDLINE directory
check_redline_directory() {
    print_status "Checking if we're in a REDLINE directory..."
    
    if [[ ! -f "main.py" ]] || [[ ! -d "redline" ]]; then
        print_error "This script must be run from the REDLINE root directory"
        print_status "Expected files: main.py, redline/ directory"
        print_status "Current directory: $(pwd)"
        print_status "Available files: $(ls -la | head -10)"
        exit 1
    fi
...
# Function to copy REDLINE to user directory
copy_redline_locally() {
    print_status "Copying REDLINE to user directory..."
    
    # Create destination directory
    mkdir -p "$REDLINE_DIR"
    
    # Copy all REDLINE files to user directory
    print_status "Copying files from $SOURCE_DIR to $REDLINE_DIR..."
    
    # Copy main files
    cp -r "$SOURCE_DIR"/* "$REDLINE_DIR/" 2>/dev/null || {
        print_warning "Some files may not have copied (this is normal)"
    }
    
    # Ensure proper ownership
    chown -R "$CURRENT_USER:$CURRENT_USER" "$REDLINE_DIR"
    
    print_success "REDLINE copied to $REDLINE_DIR"
}
...
setup_python_env() {
    ...
    if [[ -f "requirements.txt" ]]; then
        ...
        pip install flask pandas numpy matplotlib seaborn scipy scikit-learn duckdb pyarrow
    fi
    
    # Install Ubuntu-specific requirements if available
...
# Function to setup Docker images (optional)
setup_docker_images() {
    print_status "Setting up Docker images (optional)..."
    
    cd "$REDLINE_DIR"
    
    # Check if Docker files exist
    if [[ -f "Dockerfile.x86" ]] && [[ -f "Dockerfile.web" ]]; then
        # Build Docker images
        docker build --platform linux/amd64 -f Dockerfile.x86 -t redline:x86 . || {
            print_warning "Failed to build main Docker image, continuing..."
        }
        docker build --platform linux/amd64 -f Dockerfile.web -t redline-web:x86 . || {
            print_warning "Failed to build web Docker image, continuing..."
        }
        print_success "Docker images built successfully"
    else
        print_warning "Docker files not found, skipping Docker image creation"
        print_status "You can still use REDLINE with Python directly"
    fi
}
...
EOF
    ...
    # Docker startup script (if Docker is available)
...
if [[ -f "docker-compose.yml" ]]; then
    ...
    echo "❌ docker-compose.yml not found, cannot start Docker services"
...
Exec=$REDLINE_DIR/start_web.sh cycle
Icon=applications-internet
Terminal=true
Categories=Science;DataAnalysis;Network;
EOF

    # Docker shortcut (if Docker is available)
...
run_tests() {
    ...
    if source venv/bin/activate && python3 -c 'import flask; print("Flask version:", flask.__version__)' &>/dev/null; then
        print_success "Flask test passed"
    else
        print_warning "Flask test failed - Web interface may not work"
    fi
...
show_summary() {
    ...
    echo ""
    if command -v docker &> /dev/null && [[ -f "$REDLINE_DIR/docker-compose.yml" ]]; then
    ...
    if command -v docker &> /dev/null && [[ -f "$REDLINE_DIR/docker-compose.yml" ]]; then
        ...
        echo "Press Ctrl+C in the terminal where REDLINE is running"
    fi
    echo ""
    ...
    echo "• This installation uses local files (no GitHub required)"
}

# Main installation function
main() {
    print_status "Starting REDLINE local installation for current user ($CURRENT_USER)..."
