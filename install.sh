...
# Single, comprehensive installer for all platforms and environments
...
# Required dependencies based on codebase analysis
REQUIRED_PACKAGES=(
    "pandas>=2.0.0"
    "numpy>=1.24.0"
    "configparser>=5.3.0"
...
show_usage() {
    ...
    echo ""
    echo "Options:"
    echo "  --mode MODE          Installation mode: auto, minimal, full"
...
try:
    ...
    exit(1)
" &>/dev/null; then
        print_success "REDLINE modules test passed"
    else
        print_error "REDLINE modules test failed"
        return 1
    fi
