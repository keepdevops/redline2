...
def batch_download():
    ...
    if user_id:
        logger.info(f"Batch download request from authenticated user: {user_id}")
    else:
        logger.info("Batch download request from guest user (no authentication)")
    ...
    if not data:
        logger.warning(f"Batch download request from {user_label} with empty body")
        return jsonify({'error': 'Request body is required'}), 400

    if not isinstance(data, dict):
        logger.error(f"Batch download request from {user_label} with invalid data type: {type(data)}")
    ...
    if not tickers:
        logger.warning(f"Batch download request from {user_label} with no tickers")
        return jsonify({'error': 'No tickers provided'}), 400

    if not isinstance(tickers, list):
        logger.error(f"Batch download request from {user_label} has invalid tickers type: {type(tickers)}")
        return jsonify({'error': 'tickers must be an array'}), 400

    if len(tickers) == 0:
        logger.warning(f"Batch download request from {user_label} with empty tickers array")
        return jsonify({'error': 'tickers array cannot be empty'}), 400

    if len(tickers) > 500:
        logger.warning(f"Batch download request from {user_label} with too many tickers: {len(tickers)}")
    ...
    for idx, ticker in enumerate(tickers):
        if not isinstance(ticker, str):
            logger.warning(f"Batch download ticker {idx} from {user_label} has invalid type: {type(ticker)}")
            return jsonify({'error': f'Ticker at index {idx} must be a string'}), 400

        ticker = ticker.strip().upper()
        if len(ticker) == 0:
            logger.warning(f"Batch download from {user_label} has empty ticker at index {idx}")
            return jsonify({'error': f'Ticker at index {idx} cannot be empty'}), 400

        if len(ticker) > 20:
            logger.warning(f"Batch download from {user_label} has oversized ticker at index {idx}: {ticker}")
    ...
    if not isinstance(source, str):
        logger.error(f"Batch download from {user_label} has invalid source type: {type(source)}")
    ...
    if not source.startswith('custom_') and source not in valid_sources:
        logger.warning(f"Batch download from {user_label} has invalid source: {source}")
    ...
    logger.info(f"Processing batch download from {user_label}: {len(tickers)} ticker(s), source={source}")
    ...
    for i, ticker in enumerate(tickers):
        try:
            # Optimized delay for Yahoo Finance batch downloads
            # The downloader handles its own rate limiting, so we use a smaller delay here
            if i > 0 and downloader and source == 'yahoo':
                delay = 2.0  # 2 second delay (reduced from 30s - downloader handles rate limiting)
                logger.debug(f"Waiting {delay} seconds before downloading {ticker}...")
                time.sleep(delay)
            elif i > 0 and downloader:
                # Other sources may need different delays
                delay = 1.0
                time.sleep(delay)
