...
class MultiSourceDownloader(BaseDownloader):
    ...
    def __init__(self, output_dir: str = "data", massive_api_key: str = None):
        ...
        if massive_key:
            try:
                ...
                self.logger.warning(f"Massive.com downloader not available (import error): {str(e)}")
            except ValueError as e:
                self.logger.warning(f"Invalid configuration for Massive.com downloader: {str(e)}")
            except TypeError as e:
                self.logger.warning(f"Type error initializing Massive.com downloader: {str(e)}")
            except Exception as e:
                self.logger.warning(f"Unexpected error initializing Massive.com downloader: {str(e)}")
    ...
    def download_single_ticker(self, ticker: str, start_date: str = None, end_date: str = None,
        ...
        for source_name in sources_to_try:
            try:
                ...
                self.logger.error(f"Source {source_name} not found in downloaders: {str(e)}")
                self.source_stats[source_name]['failures'] += 1
            except AttributeError as e:
                self.logger.error(f"Downloader error for {source_name} downloading {ticker}: {str(e)}")
                self.source_stats[source_name]['failures'] += 1
            except Exception as e:
                self.logger.error(f"Unexpected error downloading {ticker} from {source_name}: {str(e)}")
                self.source_stats[source_name]['failures'] += 1
    ...
    def download_multiple_tickers(self, tickers: List[str], start_date: str = None, end_date: str = None,
        ...
        for i, ticker in enumerate(tickers):
            try:
                ...
                self.logger.error(f"Invalid ticker or source for {ticker}: {str(e)}")
                failed_tickers.append(ticker)
                self.stats['failed_requests'] += 1
                self.stats['total_requests'] += 1
            except AttributeError as e:
    ...
    def download_from_source(self, source: str, ticker: str, start_date: str = None,
        ...
        # Pre-validation with if-else
        if not source:
            self.logger.error("Source is empty or None")
            return pd.DataFrame()

        if not isinstance(source, str):
            self.logger.error(f"Source must be a string, got {type(source)}")
            return pd.DataFrame()
        ...
        try:
            ...
            self.source_stats[source]['failures'] += 1
            self.logger.error(f"Key error downloading {ticker} from {source}: {str(e)}")
            return pd.DataFrame()
        except AttributeError as e:
            self.source_stats[source]['failures'] += 1
            self.logger.error(f"Attribute error downloading {ticker} from {source}: {str(e)}")
            return pd.DataFrame()
        except Exception as e:
            self.source_stats[source]['failures'] += 1
            self.logger.error(f"Unexpected error downloading {ticker} from {source}: {str(e)}")
            return pd.DataFrame()
    ...
    def test_source_connectivity(self, source_name: str) -> bool:
        ...
        # Pre-validation with if-else
        if not source_name:
            self.logger.error("Source name is empty or None")
            return False

        if not isinstance(source_name, str):
            self.logger.error(f"Source name must be a string, got {type(source_name)}")
            return False

        if source_name not in self.downloaders:
            self.logger.warning(f"Source {source_name} not found in downloaders")
            return False
        ...
        try:
            ...
            self.logger.error(f"Key error testing {source_name}: {str(e)}")
            return False
        except AttributeError as e:
            self.logger.error(f"Attribute error testing {source_name}: {str(e)}")
            return False
        except Exception as e:
            self.logger.error(f"Unexpected error testing {source_name}: {str(e)}")
            return False
